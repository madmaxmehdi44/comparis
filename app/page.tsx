...
export default function Home() {
  ...
  function reset() { setData(null); setSources([]); }
  ...
  const renderGroup = (group: ComparisonGroup) => {
    ...
    return <article className="groupcard" key={group.key}>
      ...
      <div className="offers">{group.offers.map((offer, index) => <a className="offer" href={offer.url} target="_blank" rel="noreferrer" key={`${offer.sourceId}-${offer.url}-${index}`}><span><b>{offer.source}</b><small>{statusText[offer.status]} · {Math.round(offer.confidence * 100)}٪</small></span><strong>{toman(offer.price)}</strong></a>)}</div>
    </article>;
  };
