...
function safeHttpUrl(raw: string): URL {
  ...
  if (blockedHosts.has(host) || host.endsWith('.localhost') || /^\d+(?:\.\d+){3}$/.test(host)) throw new Error('unsafe target rejected');
  return url;
}
...
async function withPage(options: PlaywrightLabOptions, work: (page: Page, context: BrowserContext) => Promise<Record<string, unknown>>) {
  ...
  try {
    await page.goto(safeHttpUrl(options.url).toString(), { waitUntil: 'domcontentloaded', timeout: options.timeoutMs });
...
export async function runPlaywrightLab(options: PlaywrightLabOptions): Promise<PlaywrightLabResult> {
  ...
  try {
    const execution = await withPage(options, async (page, context) => {
      switch (options.mode) {
        ...
        case 'locators': {
          ...
          const items = await locator.evaluateAll((els, max) => els.slice(0, Number(max)).map((el) => ({ text: (el.textContent || '').replace(/\s+/g, ' ').trim(), href: (el as HTMLAnchorElement).href || undefined, tag: el.tagName.toLowerCase() })), options.maxItems ?? 20);
          return { selector, count, items };
        }
        case 'evaluate-dom': {
          const selector = options.selector || '[itemtype*="Product"], [data-product], article';
          const items = await page.locator(selector).evaluateAll((els, max) => els.slice(0, Number(max)).map((el) => ({ text: (el.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 500), html: el.outerHTML.slice(0, 1200) })), options.maxItems ?? 10);
          return { selector, items };
        }
        case 'network': {
          const requests: string[] = [];
          const responses: Array<{ status: number; url: string }> = [];
          page.on('request', (request) => { if (['xhr', 'fetch'].includes(request.resourceType())) requests.push(`${request.method()} ${request.url()}`); });
          page.on('response', (response) => { if (['xhr', 'fetch'].includes(response.request().resourceType())) responses.push({ status: response.status(), url: response.url() }); });
        ...
        case 'screenshot': {
          ...
          return { screenshotBytes: buffer.length, viewport: page.viewportSize(), captured: true };
        }
        case 'responsive': {
          await page.setViewportSize({ width: 390, height: 844 });
          return { mobile: await page.locator('body').evaluate((body) => ({ width: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth, overflowX: document.documentElement.scrollWidth > document.documentElement.clientWidth, textLength: body.innerText.length })) };
        }
        case 'locale':
          return { locale: await page.evaluate(() => navigator.language), timezone: await page.evaluate(() => Intl.DateTimeFormat().resolvedOptions().timeZone), colorScheme: await page.evaluate(() => matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light') };
        case 'storage': {
          const storage = await context.storageState();
          return { cookies: storage.cookies.length, origins: storage.origins.map((origin) => ({ origin: origin.origin, localStorageEntries: origin.localStorage.length })) };
        }
        case 'frames':
        ...
        case 'click-and-pagination': {
          ...
          if (found) await next.click({ timeout: Math.min(options.timeoutMs, 5000) }).catch(() => undefined);
          await page.waitForLoadState('domcontentloaded', { timeout: Math.min(options.timeoutMs, 7000) }).catch(() => undefined);
          return { selector, found, finalUrl: page.url() };
        }
        case 'smart-extract': {
              ...
              try {
                ...
                for (const root of roots) {
                  ...
                  for (const item of graph) {
                    ...
                    for (const offer of offers) if (offer && typeof offer === 'object') output.push({ name: item.name, price: offer.price ?? offer.lowPrice, currency: offer.priceCurrency, url: offer.url });
                  }
                }
              } catch {}
        ...
        case 'full-diagnostics': {
          const [links, buttons, controls] = await Promise.all([page.getByRole('link').count(), page.getByRole('button').count(), page.locator('input,textarea,select').count()]);
          return { url: page.url(), title: await page.title(), viewport: page.viewportSize(), links, buttons, formControls: controls, frames: page.frames().length, contentLength: (await page.content()).length };
        }
      }
    });
    ...
    return { mode: options.mode, url: options.url, finalUrl: options.url, title: '', durationMs: Date.now() - started, status: 'failed', details: {}, errors: [error instanceof Error ? error.message : 'Playwright execution failed'] };
  }
}
