  ...
  try {
    const execution = await withPage(options, async (page, context) => {
      switch (options.mode) {
        ...
        case 'responsive': { await page.setViewportSize({ width: 390, height: 844 }); return { mobile: await page.locator('body').evaluate((body) => { const element = body as HTMLElement; return { width: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth, overflowX: document.documentElement.scrollWidth > document.documentElement.clientWidth, textLength: element.innerText.length }; }) }; }
