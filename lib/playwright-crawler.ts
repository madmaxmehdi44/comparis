...
const MAX_OFFERS = 30;
const UA = 'ComparisPlaywright/1.1';
...
export async function crawlWithPlaywright(source: SourceDefinition, query: string, timeoutMs = 7000): Promise<SourceResult> {
  ...
  try {
    ...
    page.on('response', (response) => { if (['xhr', 'fetch'].includes(response.request().resourceType())) responses.push({ url: response.url(), status: response.status(), type: response.request().resourceType() }); });
    await page.goto(target, { waitUntil: 'domcontentloaded', timeout: timeoutMs });
    await page.waitForLoadState('networkidle', { timeout: Math.min(timeoutMs, 4000) }).catch(() => undefined);
